...
IDF), read straight off the lexical index (`corpus_fts`). They are pure measurement — no answering,
no condensation — which is why they sit here rather than inside either consumer: sage's recall path
(`content_search`) and astra's document keying (`doc_index.extract_terms`) must agree about which
words carry a question, so they read one module's measure from the same store. One path, no
stop-list, nothing hand-authored.

`sage/content_search.py` imports the whole cluster (`terms`/`_q`/`_df`/`_salient`/`_corpus_rows`/
`_match_expr`); `sage/canon.py` takes `terms`; `astra/doc_index.py` takes `_salient`.

Provenance
──────────
Vendored from `agience-mantle`, `src/mantle/ontology/corpus_stats.py` (Apache-2.0, Copyright Ikailo
Inc.), at commit `9768c5f`; mantle removed it in `a7cfd0c` alongside the FTS index it reads, and it
follows that index here. `agience-ember` carries its own copy at `ember/ontology/corpus_stats.py`
for the same reason — see the byte-identity note in `corpus_fts.py`, which applies to this pair
too. Copied
unmodified apart from this note and the `fts` import path.
"""
...
import agience_chorus.corpus_fts as _fts   # the index — one implementation, vendored beside this one
