...
CHORUS = HERE.parents[4]                             # …/<repo>/src/agience_chorus/seraph/tests -> <repo>
...
def _workspace() -> pathlib.Path:
    ...
    for cand in (HERE.parents[5], CHORUS / ".siblings"):
        if all((cand / n).is_dir() for n in needed):
            return cand
    return HERE.parents[5]                           # nothing satisfies it; fail loudly downstream


WORKSPACE = _workspace()
BUNDLES = CHORUS / "src" / "agience_chorus" / "bundles"   # built here, from chorus source;
...
from agience_chorus.seraph import bundling  # noqa: E402

pytestmark = pytest.mark.skipif(
    not (WORKSPACE / "agience-chorus" / "src" / "agience_chorus" / "seraph"
         / "bundle_spec.json").is_file(),
    reason="this repository's own src/agience_chorus/seraph/bundle_spec.json is missing")
...
def test_the_organon_REFUSES_a_moved_module_instead_of_searching_by_basename():
    ...
    with tempfile.TemporaryDirectory() as d:
        root = pathlib.Path(d)
        (root / "agience-chorus" / "src" / "agience_chorus" / "bundles").mkdir(parents=True)
        (root / "agience-chorus" / "src" / "agience_chorus" / "seraph").mkdir(parents=True)
        (root / "agience-chorus" / "src" / "agience_chorus" / "seraph" / "bundle_spec.json").write_text(json.dumps(tmp_spec),
                                                                  encoding="utf-8")
...
def test_LANDING_is_explicit_and_writes_the_same_bytes():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        (root / "agience-chorus" / "src" / "agience_chorus" / "seraph").mkdir(parents=True)
        shutil.copy(
            WORKSPACE / "agience-chorus" / "src" / "agience_chorus" / "seraph" / "bundle_spec.json",
            root / "agience-chorus" / "src" / "agience_chorus" / "seraph" / "bundle_spec.json")
        # only the `bundling` group's source is needed for a one-group land, and it has to sit at
        # the path the spec declares — which is inside the package now.
        shutil.copy(SERAPH / "bundling.py",
                    root / "agience-chorus/src/agience_chorus/seraph/bundling.py")

        need = {"groups": ["bundling"], "land": True}
        got = bundling.bundle_condense_handler(str(root))(need)
        assert got["landed"] == ["bundling"], got
        landed = json.loads((root / "agience-chorus" / "src" / "agience_chorus" / "bundles" / "bundling.json")
