...
def _ingest(store: Any, root: str, *, author: str = "sage-canon",
    ...
    if arts:
        import agience_chorus.corpus_fts as _fts                 # lazy: keep canon.py import-light
...
def retrieve_cited(corpus: Any, query: str, *, k: int = 6) -> List[Dict[str, Any]]:
    ...
    try:                                             # 1 · the declared seam, if a host bound one
        ...
        try:                                         # 2 · robust: cross-persona host vs in-persona
            from agience_chorus.sage import content_search as _cs   # (chorus/src on path) — sage's own retrieval tekton
        except ImportError:
...
def _fired_names(text: str) -> List[str]:
    ...
    from agience_chorus.corpus_stats import terms as _terms
