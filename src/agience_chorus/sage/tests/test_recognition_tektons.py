...
def run_phase(phase: str, tmp: str) -> dict:
    ...
    # The child inherits the environment, never the parent's runtime `sys.path`. Under an installed
    # chorus it would find `agience_chorus` anyway; under a bare checkout it is pytest that put
    # `src/` on the path, and that knowledge dies at the process boundary. So the parent hands its
    # own path down rather than assuming either arrangement.
    env = dict(os.environ)
    env["PYTHONPATH"] = os.pathsep.join(
        [p for p in sys.path if p] + [x for x in (env.get("PYTHONPATH"),) if x])
    proc = subprocess.run([sys.executable, PROBE, phase, tmp],
...
def test_only_one_grounding_capability_is_published(plane):
    ...
    it was reached.
    """
    from agience_chorus.sage import recognition as R
