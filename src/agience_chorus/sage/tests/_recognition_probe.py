...
def _seam_state() -> str:
    ...
    from agience_chorus._host_seams import HostSeamUnfilled, seam
    try:
...
def phase_provide(tmp: str) -> None:
    ...
    from agience_chorus.sage import recognition
...
def phase_paths(tmp: str) -> None:
    ...
    from agience_chorus.sage import recognition
    from agience_chorus._host_seams import resolve as seam
...
def phase_control(tmp: str) -> None:
    ...
    from agience_chorus.sage import recognition  # noqa: F401  — the tekton module itself must import with ember unreachable
...
def _requester(tmp: str):
    from prism.carriers import StoreCarrier
    from agience_chorus.reach_wiring import reactor
    store = _store(tmp)
    carrier = StoreCarrier(store)
