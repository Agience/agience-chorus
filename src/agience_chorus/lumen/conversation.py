...
def _region(d, relation: str, side: str) -> Optional[str]:
    ...
    try:
        ...
        try:                                              # robust: cross-persona host vs in-persona process
            from agience_chorus.lumen import templates                   # (chorus/src on path)
        except ImportError:
            from agience_chorus.lumen import templates                              # (lumen/ on path — moved with this tekton)
        pts = []
...
def _quantitative(text: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        from agience_chorus.lumen import reasoning as _R
    except ImportError:
        import agience_chorus.lumen.reasoning as _R
    try:
