...
/**
 * Build a RecordDefinition from a type.json `ui.record` block — the declarative,
 * schema-driven path. Icon comes from the resolved type definition (the registry-
 * resolved react-icons component), so a record type needs no frontend code: it
 * declares `ui.viewer: "record"` + `ui.record` and gets view/edit/save for free.
 */
function recordDefinitionFromConfig(
  cfg: RecordConfig,
  def: ContentTypeDefinition,
): RecordDefinition {
  return {
    label: def.label,
    icon: def.icon as ComponentType<{ size?: number; className?: string }>,
    titlePath: cfg.titlePath,
    subtitlePath: cfg.subtitlePath,
    badgePath: cfg.badgePath,
    fields: cfg.fields,
...
export default function RecordViewer({
  ...
  // The record schema is fully declarative — it comes from the type's own
  // `ui.record` (served by the platform at runtime). No type-specific knowledge
  // lives here; a type without `ui.record` gets a generic key-derived fallback.
  const typeDef = useMemo(() => getContentType(artifact), [artifact]);
  const fromTypeJson = Boolean(typeDef.record);
  const definition = fromTypeJson
    ? recordDefinitionFromConfig(typeDef.record as RecordConfig, typeDef)
    : fallbackDefinition(mime);
  const isWorkspaceArtifact = useMemo(
    () => artifacts.some((workspaceArtifact) => String(workspaceArtifact.id) === String(artifact.id)),
    [artifacts, artifact.id]
  );
  // Declarative records are editable wherever viewed — only principals with
  // update access can open them, and the backend authorizes the PATCH regardless.
  // The generic fallback keeps the workspace gate (no declared editability).
  const canEdit = isWorkspaceArtifact || fromTypeJson;
    ...
    <div className="flex h-full flex-col gap-4 overflow-y-auto bg-white p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {canEdit && (editableFields.length > 0 || editableCollections.length > 0 || Boolean(definition.titlePath)) && (
            isEditing ? (
              <>
                <button
                  type="button"
