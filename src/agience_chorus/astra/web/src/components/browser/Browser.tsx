...
interface BrowserProps {
  ...
  /** Precision value for search results */
  searchPrecision?: number;
  /** Callback when precision changes */
  onSearchPrecisionChange?: (value: number) => void;
}
...
export default function Browser({ 
  ...
  onSearchSortChange,
  searchPrecision = 0.5,
  onSearchPrecisionChange,
}: BrowserProps) {
  // selectedArtifactId is accepted for interface parity but not read here; void it to satisfy the unused-variable lint rule.
  ...
  // setDisplayedArtifacts is exposed by WorkspaceProvider; some context implementations may not define it.
  ...
  useEffect(() => {
    ...
    if (activeSource.type === 'workspace') {
      ...
      // MCP-server sources have no artifacts list.
      setSourceArtifacts([]);
    }
  ...
  // Workspaces are primarily manually ordered via drag-reorder.
  ...
  const {
    ...
    // recentSearches, // Unused here — search input lives in MainHeader
    // updateSearch, // Unused here — global search runs through AdvancedSearch
    clearSearch,
    isSearchActive,
  ...
  const convertSearchHitsToArtifacts = useCallback(async (hits: SearchHit[]): Promise<Artifact[]> => {
    // Search returns IDs only; artifacts must be fetched from source.
    // Strategy:
  ...
  const filteredArtifacts = useMemo(() => {
    ...
    // Strip the workspace container artifact card. The workspace container
    // artifact uses the workspace ID as both id/root_id and should never
  ...
  // Commit the selected draft(s) — one card or many. The backend skips any
  // non-draft ids, but we pass only drafts so the toast/count is accurate.
  const handleBulkCommit = useCallback(() => {
    const draftIds = bulkSelectedArtifacts
      .filter(a => a.state === 'draft')
      .map(a => String(a.id));
    if (draftIds.length === 0) return;
    commitCurrentWorkspace({ artifact_ids: draftIds });
    unselectAllArtifacts();
  }, [bulkSelectedArtifacts, commitCurrentWorkspace, unselectAllArtifacts]);
  ...
  // Double-click tab to rename workspace. Also updates the workspace container
  ...
  // Drag-depth tracking for the workspace's own visual feedback (independent of CardGrid's).
  ...
  // dragDepth and workspaceDragKind are set by the handlers above for consumers that need to
  // know whether a file or artifact drag is over the workspace (e.g. BrowserHeader drop targets).
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <BrowserHeader
        ...
        onPrecisionChange={onSearchPrecisionChange}
