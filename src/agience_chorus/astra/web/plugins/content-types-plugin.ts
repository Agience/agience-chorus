...
/**
 * A single field in a generic-record viewer schema (the `ui.record` block).
 * `path` is a dot-path into the artifact's parsed `context`.
 */
...
/**
 * Declarative config for the generic `_record` viewer. A type opts in by setting
 * `ui.viewer: "record"` and declaring this block — the viewer then renders a
 * schema-driven view/edit/save form with no per-type code.
 */
export interface RecordConfig {
  ...
  collections?: RecordFieldConfig[];
  objects?: RecordFieldConfig[];
}
...
export interface RawPresentationJson {
  ...
  /** Generic-record viewer schema (view/edit/save). See {@link RecordConfig}. */
...
export interface ResolvedContentType {
  ...
  container_variant: string | null;
  /** Generic-record viewer schema (view/edit/save). Null when the type doesn't use it. */
  record: RecordConfig | null;
...
function scanTypesRoot(
  ...
  for (const top of topEntries) {
    ...
    for (const sub of subEntries) {
      ...
      // This plugin scans core primitives (package/types) only — a synchronous
      // build-time bootstrap. Server-owned types (and their viewers) are not
      // scanned here; they arrive at runtime from the platform's /types feed.
      // A type's viewer pointer (ui.resource_uri/resource_server), when present,
      // is declared in its own type.json `ui` block — nothing is inferred here.
...
function buildDefinitions(typesRoots: string[]): ResolvedContentType[] {
  ...
  // Core primitives only (a single root: package/types). Cross-root overrides
  // (core < server) are resolved by the platform at runtime, not here.
  ...
  for (const contentType of raw.keys()) {
    ...
    results.push({
      ...
      record: (m.record as RecordConfig | null | undefined) ?? null,
