"""A lightweight ontology browser — a FACET, so it lives in chorus with the other tools.

It renders what the running engine holds, and reaches the engine through HOST SEAMS rather than by
importing it. Chorus may not import ember (`src/tests/test_chorus_does_not_import_ember.py`), and
the rule is architecture rather than packaging: ember is the workflow engine, chorus is the tools,
and a tool that links the engine cannot be served by any other host.

It was `ember/facets/browse.py` until the facets moved. Living inside the engine let it import
`ember.genesis`, `ember.runtime.improve`, `ember.surface.stats` and `ember.runtime.pool` directly;
those are now the `genesis` / `improve` / `stats` / `pool` seams, bound by
`ember/runtime/seams.py`. Same-origin with the host that serves it, so the page and its API share
a CSP origin.
...
def _seam(name: str):
    """The module the host bound to `name`, or a clear refusal.

    `HostSeamUnfilled` is re-raised as `RuntimeError` naming the seam: a facet reached with no
    engine behind it is a deployment fact an operator can act on, not a missing import.
    """
    from _host_seams import HostSeamUnfilled, resolve
    try:
        return resolve(name)
    except HostSeamUnfilled as exc:
        raise RuntimeError(
            "the %r seam is unfilled, so this facet cannot read the engine. A host binds it — "
            "ember does, in ember/runtime/seams.py." % name) from exc
...
def api_artifact(bundle, artifact_id: str) -> dict:
    ...
    # the fields.
    from prism.runner import evolution            # a bundle group: prism loads it, no host needed
...
def dashboard_page(bundle) -> str:
    ...
    _stats = _seam("stats")
    from aria.facets.dashboard_render import render_dashboard   # a sibling facet, here
...
def library_view(bundle, *, root: str = "agience-pharos", refresh: bool = False,
    ...
    """
    import time
    from prism.runner import docs_ops                  # the single distribution path
    key = (root, resolution)
