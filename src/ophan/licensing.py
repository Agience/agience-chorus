"""Licensing artifacts — issue, verify and preflight signed licenses and activation leases.

Key paths and the signing key id come from `prism.trust.key_manager`, not from `origin.config`:
`LICENSING_PUBLIC_KEYS_PATH`, `LICENSING_PRIVATE_KEY_PATH`, and `LICENSING_SIGNING_KEY_ID` are
managed by key_manager and are not environment-configurable, so ophan's licensing path does not
depend on the identity service. A per-deployment override belongs in key_manager, as a real,
read setting, rather than as a separate constant here.
"""
...
def _packaging_root() -> Path:
    """The machine-readable licensing policy tree (profiles + control classes).

    It ships with ophan, in the chorus image, at ``ophan/policy/`` — ophan is the
    runtime consumer, so its data must be present in the same container. Overridable
    via ``LICENSING_POLICY_ROOT`` for out-of-tree / mounted-config deployments."""
    override = os.getenv("LICENSING_POLICY_ROOT")
    if override:
...
def load_trust_anchors() -> Dict[str, Any]:
    path = get_licensing_trust_anchors_path()
...
def _canonicalize_signed_payload(payload: Dict[str, Any]) -> bytes:
    ...
    return _jcs_string(unsigned).encode("utf-8")
...
def _load_signing_private_key() -> Ed25519PrivateKey:
    path = get_licensing_private_key_path()
    if not path.exists() or not path.is_file():
        raise RuntimeError(
...
def _resolve_signing_key_id(signing_key_id: Optional[str] = None) -> str:
    kid = signing_key_id or get_licensing_key_id()
    if not kid:
        raise RuntimeError(
...
def digest_signed_payload(payload: Dict[str, Any]) -> str:
    canonical = _jcs_string(payload).encode("utf-8")
...
def verify_signature(payload: Dict[str, Any], trust_anchors: Optional[Dict[str, Any]] = None) -> SignatureVerificationResult:
    ...
    if alg != "EdDSA":
        return SignatureVerificationResult(valid=False, key_id=kid, algorithm=alg, message=f"Licensing signatures are EdDSA; this license presents '{alg}'.")
