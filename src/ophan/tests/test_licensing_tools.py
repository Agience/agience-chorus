...
# 2026-08-26: loaded by path with no `sys.modules` registration, so every import built a
# SECOND module object. `src/_persona.py` is the one home for this and is idempotent.
...
async def test_issue_license_rejects_missing_entitlements(monkeypatch):
    ...
    # The refusal is parseable JSON, not prose: every ophan tool returns `json.dumps({"error": ...})`
    # on the failure path, matching the shape of a success response, so a caller can always
    # `json.loads(result)` rather than distinguishing a refusal from a malformed success by prefix.
    payload = json.loads(result)
    assert "error" in payload, payload
    assert "Missing required entitlements" in payload["error"]
    assert "licensing_operations" in payload["error"]
