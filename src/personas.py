...
def _load_persona_modules() -> dict[str, ModuleType]:
    if _LOCAL_LAYOUT:
        log.info("Local layout detected — loading persona modules from %s%s", HOST_DIR,
                 f" (crystals: {', '.join(_SELECTED)})" if _SELECTED else " (all crystals)")
        _all = {
            "aria": ("servers_host_aria_server", HOST_DIR / "aria" / "server.py"),
            "astra": ("servers_host_astra_server", HOST_DIR / "astra" / "server.py"),
            "sage": ("servers_host_sage_server", HOST_DIR / "sage" / "server.py"),
            "iris": ("servers_host_iris_server", HOST_DIR / "iris" / "server.py"),
            "ophan": ("servers_host_ophan_server", HOST_DIR / "ophan" / "server.py"),
            "seraph": ("servers_host_seraph_server", HOST_DIR / "seraph" / "server.py"),
            "lumen": ("servers_host_lumen_server", HOST_DIR / "lumen" / "server.py"),
        }
        ...
        unknown = [n for n in _SELECTED if n not in _all]
        if unknown:
            raise RuntimeError(
                "CHORUS_CRYSTALS names %s, which is not a persona crystal here; known: %s"
                % (", ".join(unknown), ", ".join(sorted(_all))))
        return {n: _load_module(*spec) for n, spec in _all.items() if _wanted(n)}
    log.info("Flat layout detected — importing persona modules directly")
    import aria_server  # type: ignore[import-not-found]
    import astra_server  # type: ignore[import-not-found]
    import iris_server  # type: ignore[import-not-found]
    import lumen_server  # type: ignore[import-not-found]
    import ophan_server  # type: ignore[import-not-found]
    import sage_server  # type: ignore[import-not-found]
    import seraph_server  # type: ignore[import-not-found]
    ...
    _flat = {
        ...
        "iris": iris_server,
        "ophan": ophan_server,
        "seraph": seraph_server,
        "lumen": lumen_server,
    }
    return {n: m for n, m in _flat.items() if _wanted(n)}
...
def _build_mount_app(name: str, mod: ModuleType) -> Any:
    ...
    if name == "aria":
        ...
        combined = FastAPI()
        try:
            import web_bff  # aria-local (the persona dir is on the path)
    ...
    if name == "sage":
        ...
        combined = FastAPI()
        try:
            import pharos  # sage-local (the persona dir is on the path)
    ...
    try:
        if _LOCAL_LAYOUT:
            stream_mod = _load_module(
                "servers_host_astra_stream_routes", HOST_DIR / "astra" / "stream_routes.py"
            )
        else:
            import stream_routes as stream_mod  # type: ignore[import-not-found]
...
def _persona_manifest(name: str, mod: ModuleType) -> tuple:
    ...
    none (aria/iris/ophan/…) has no manifest and returns empty lists. Loaded by path with a unique
    module name (the files are all named ``manifest``); the persona dir is already on
    ``sys.path`` (see ``_load_module``), so the manifest's bare organon imports resolve.
    ...
    mpath = Path(getattr(mod, "__file__", "")).resolve().parent / "manifest.py"
    if not mpath.exists():
        return [], []
    try:
        m = _load_module(f"servers_host_{name}_manifest", mpath)
...
def _wire_pharos_facet() -> None:
    ...
    if store is None:
        ...
        return
    try:
        import canon    # sage-local
        import pharos   # sage-local
...
def _wire_conversation_carrier() -> None:
    ...
    try:
        ...
        if os.getenv("READ_ONTOLOGY") or os.getenv("READ_COLLECTION"):
            from lumen.reading import chat as _readchat
        ...
        from lumen import reach_provider
