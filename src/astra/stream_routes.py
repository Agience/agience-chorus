...
Platform operations are performed via HTTP calls to Mantle. Each call signs
its own platform JWT via the chorus service identity — no token exchange,
no shared PLATFORM_INTERNAL_SECRET.
...
# ---------------------------------------------------------------------------
# Platform auth — sign a chorus platform JWT per request
# ---------------------------------------------------------------------------


def _sign_platform_token() -> str:
    """Sign a fresh platform JWT identifying Astra to Mantle.
...
async def _headers(bearer: str | None = None) -> dict[str, str]:
    ...
    otherwise signs a fresh chorus platform JWT for Astra."""
    h: dict[str, str] = {"Content-Type": "application/json"}
    token = bearer if bearer is not None else _sign_platform_token()
    if token:
...
async def _upload_file_to_artifact(workspace_id: str, artifact_id: str, file_path: str, content_type: str = "video/mp4") -> bool:
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            resp = await client.post(
                artifact_url(MANTLE_URI, artifact_id, "upload-initiate"),
                headers=await _headers(),
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        resp = await client.patch(
            artifact_url(MANTLE_URI, artifact_id, "upload-status"),
            headers=await _headers(),
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            verify_resp = await client.get(
                artifact_url(MANTLE_URI, source_artifact_id),
