...
This keeps the Tailwind config free of hardcoded values — every entry under `theme.extend.colors` is
`hsl(var(--…))`. Components that still use literal palette classes (`text-gray-500`, `border-gray-200`)
do not follow the variables and are not reached by a change to `src/index.css`.
