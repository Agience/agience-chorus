...
#
# The emitted KEYS must match src/config/runtime.ts:
#   originUri  — Origin (auth / identity / OIDC / setup)   e.g. https://origin.agience.ai
#   mantleUri  — Mantle (artifacts / search / events)      e.g. https://mantle.agience.ai
#   backendUri — legacy alias for mantleUri (runtime.ts falls back to it)
#   crystalUri — Crystal (content-type op gateway)         e.g. https://crystal.agience.ai
#   clientId, title, favicon
#
# Reads the VITE_-prefixed names (build-time names reused as runtime env),
# falling back to bare names, then to local-dev ports.
set -eu
...
CRYSTAL_URI="${VITE_CRYSTAL_URI:-${CRYSTAL_URI:-http://localhost:8085}}"
CLIENT_ID="${VITE_CLIENT_ID:-${CLIENT_ID:-agience-client}}"
...
window.__AGIENCE_CONFIG__ = Object.freeze({
  ...
  crystalUri: "$(json_escape "$CRYSTAL_URI")",
