...
describe('runtime config branding', () => {
  ...
  it('defaults crystalUri (the op gateway) and honours a runtime override', () => {
    expect(getRuntimeConfig().crystalUri).toBe(
      import.meta.env.VITE_CRYSTAL_URI || 'http://localhost:8085',
    );

    (window as { __AGIENCE_CONFIG__?: unknown }).__AGIENCE_CONFIG__ = {
      crystalUri: 'https://my.agience.ai/gateway',
    };
    expect(getRuntimeConfig().crystalUri).toBe('https://my.agience.ai/gateway');
  });
