...
// No LLM Keys settings section: this app has no model configuration surface.
type SettingsSection = 'profile' | 'general' | 'demo-data';
...
export default function SidebarEnhanced({ 
  ...
  const [resourcesDrawer, setResourcesDrawer] = useState<ResourcesDrawerState | null>(null);
  ...
  // Part of the sidebar search prop contract but not read by this component.
  void onSearchSortChange;
  void onSearchPrecisionChange;
  ...
  return (
    <div ref={sidebarRef} className="flex flex-col h-full bg-white relative z-20">
          ...
          <div className="pb-1">
            ...
            {(() => {
              ...
              Object.entries(mcpServerInfos).forEach(([serverId, serverInfo]) => {
                ...
                // by-time and by-author render a flat, ungrouped list
                return allResources.map((resource) => {
                  const isDrawerOpen = resource.isCollection && resourcesDrawer?.collectionId === resource.id;
                  const resourceIcon = resource.icon;
      ...
      {/* Tools drawer (bottom-anchored panel) */}
      {toolsDrawer && (
        <div 
      ...
      {/* Resources drawer (bottom-anchored panel) */}
      {resourcesDrawer && (
        <div 
          className="fixed bg-white border-r border-b border-border shadow-sm z-10 transition-opacity duration-150"
          style={{
