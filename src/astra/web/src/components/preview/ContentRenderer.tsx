...
/**
 * ContentRenderer - Renders artifact content based on MIME type, filename
 * extension, or (as a fallback) the content's own structure.
 *
 * Media types (image/video/audio/pdf) with an absolute http(s) URI render
 * as embedded media. Everything else renders as text: JSON, JavaScript,
 * Python, XML, and YAML render as labeled code blocks, markdown renders
 * via the TipTap-based MarkdownRenderer, and unrecognized types fall back
 * to plain text. When content is empty and a URI is provided for a
 * non-media type, the text is fetched from that URI.
 */
