...
interface CardGridProps {
  ...
  editable?: boolean;
  inPanel?: boolean;
  fillHeight?: boolean; // whether the grid fills available height
...
export default function CardGrid({
  ...
  editable = true,
  inPanel = false,
  fillHeight = false, // default to natural height
  isSelected,
  activeSource,
  ...
  const applyOrder = (
    ...
    // If artifacts are external, notify the parent first and return; the parent updates the
    // artifacts prop and this component re-renders from that.
  ...
  const handleGridDrop = (e: React.DragEvent) => {
    ...
    if (draggable && isArtifactsDragLike(e)) {
      ...
      if (dragged && dragged.length) {
        ...
        // If artifacts are external, notify the parent and return without reordering.
        if (notPresent.length > 0 && onArtifactDrop) {
