...
vi.mock('../api', () => ({
  ...
  // `getList` is a NAMED export, not a member of the default — `mcp.ts` does
  // `import api, { getList } from './api'`. Mocking it under `default` leaves the
  // named import undefined, which fails the whole MODULE rather than one test.
  getList: vi.fn(),
}));
...
const mockList = getList as unknown as ReturnType<typeof vi.fn>;
...
describe('api/mcp — listWorkspaceMCPServers', () => {
  ...
  it('calls GET /artifacts/visible with mcp-server content_type and returns mapped server list', async () => {
    ...
    mockList.mockResolvedValueOnce(artifacts);

    const result = await listWorkspaceMCPServers('ws-123');

    expect(mockList).toHaveBeenCalledWith('/artifacts/visible', {
...
describe('api/mcp — importMCPResources (Phase 7D: artifact-native dispatch)', () => {
  ...
  it('resolves a server name to a UUID before posting resources_import', async () => {
    mockList.mockResolvedValueOnce([{ id: '8ce77110-f1af-4ab3-86d8-f84305888008', name: 'Agience Core', content_type: 'application/vnd.agience.mcp-server+json', context: '{}', content: '', state: 'committed' }]);
...
describe('api/mcp — readMCPResource (Phase 7D: artifact-native dispatch)', () => {
  ...
  it('resolves the workspace server name to a UUID before resources_read', async () => {
    ...
    mockList.mockResolvedValueOnce([{ id: 'cf759fa7-1d53-4867-8c20-2c6d92fe4d0d', name: 'Seraph', content_type: 'application/vnd.agience.mcp-server+json', context: '{}', content: '', state: 'committed' }]);
...
describe('api/mcp — proxyToolCall', () => {
  ...
  it('resolves a server name to a UUID before invoke', async () => {
    mockList.mockResolvedValueOnce([{ id: '641e9cfb-2b57-4515-b912-9d0a07eb6225', name: 'Ophan', content_type: 'application/vnd.agience.mcp-server+json', context: '{}', content: '', state: 'committed' }]);
    ...
    expect(mockPost).toHaveBeenCalledWith(
      '/artifacts/641e9cfb-2b57-4515-b912-9d0a07eb6225/op/invoke',
