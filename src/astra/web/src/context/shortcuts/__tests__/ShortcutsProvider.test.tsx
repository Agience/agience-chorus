...
describe('ShortcutsProvider', () => {
  ...
  it('registerShortcut adds to the group and returns an unregister fn', () => {
    ...
    // ctx is a snapshot from the original provider, so the assertion accepts
    // either an unpopulated group (pre-rerender snapshot) or the registered shortcut.
  ...
  it('unregister removes the shortcut and deletes the group when empty', () => {
    ...
    // ctx.groups is a closure on the first render, so group membership is verified
    // indirectly: a keydown fires the handler before unregister and not after.
    const handler = vi.fn();
    act(() => {
      ctx.registerShortcut({
        id: 'test:solo-fire',
  ...
  it('openDialog / closeDialog / toggleDialog flip the flag', () => {
    ...
    // isDialogOpen is state on the original provider's closure; this asserts
    // the dialog control methods exist and can be called without error.
