...
export async function agentHandler(state: PaletteState, ctx: AgentRunContext): Promise<PaletteState> {
  ...
  if (!ctx.workspaceId) {
    ...
    // Generic tool dispatch is ambiguous under the artifact-invoke model: a tool
    // name alone doesn't identify which server owns it, and invocation requires
    // a server id (POST /artifacts/{server_id}/invoke). extract_information is
    // the one tool wired up as a known, single-server case.
    outputArtifacts.push({
      id: 'palette-output',
      content:
