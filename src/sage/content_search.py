...
from answer_shape import Answer          # sage-local; ember.runtime.engine exports no Answer type
...
from corpus_stats import terms, _q, _df, _corpus_rows, _salient, _match_expr
import corpus_fts as _fts   # the index — one implementation, in the store
...
from _host_seams import seam as _seam, resolve as _resolve_seam
...
def _describe(store, art: dict, ct: str, content: str) -> str:
    ...
    try:                                         # sage-local, not ember's
        from sage import offer as _offer         # (chorus/src on path)
    except ImportError:
        import offer as _offer                   # (sage/ on path)
    a = dict(art)
...
def _condense_reached(store, query: str, cand):
    ...
    try:                                             # robust: cross-persona host vs in-persona process
        from sage import condense as _cd             # (chorus/src on path)
    except ImportError:
        import condense as _cd                       # (sage/ on path)
...
def wedge_answer(store, query: str) -> Optional[Answer]:
    ...
    if not claim:
        return None
    try:                                             # robust: cross-persona host vs in-persona process
        from sage import wedge                       # (chorus/src on path)
    except ImportError:
        import wedge                                 # (sage/ on path)
